...
Between Debian Maintainer, Debian Developers, Sponsors, members of the
General Assembly, Technical Committee, and the foundation, Debian has
perhaps more bureaucratic machinery than some US states and perhaps
even some European nations.  All of this managerial infrastructure and
distribution means that its virtually impossible for Debian to ever
suffer a discontinuity in leadership, since someone will always be
available.  Unfortunately, it also means that like state governments
and most members of the EU, its very difficult to get anything done
quickly.  Worse, the complexity can lead to distrust in the leadership
when things happen in uncommon ways or the system appears to not honor
a majority will, such as the trouble around the switch to systemd and
the struggle for decision making power between the technical committee
and the general assembly.  Debian's managerial machinery is also not
cheap, there are entire sub teams dedicated to managing the
infrastructure surrounding these human processes and the
...
the same time, a German Linux magazine wanted to do a rather length
...
and function as advertised, and that it would continue to do so.
...
insolvable differences of opinion in this area.

We procured the resources to, and maintain the capability to fork if
necessary, brand and completely shut off Void.  Since the software
is Open Source and freely licensed, a name change would be about the
worst that would happen, but the ideals of Void would live on and
systems could be transparently migrated to a new name and new
repositories.  Fortunately it hasn't come to this yet.
...
Fortunately someone had bought this name and new of the project, and
had just been waiting for a contact to arrange the transfer.  We
...
account on already.  Incidentally its perhaps one of the least
...
not solved in a way that doesn't introduce security concerns.

So we were stuck with GitHub and no way to get control of our
organization.  As we later would discover, they wanted to see some
...
excessive to the reader for how this would have worked out to get a
...
a Delaware cooperation that's been sued a few times by the EU.
...
have exactly the same problem again.
